...
class MiraNoBnNoDoNoSigmoidOnPredictLayer(Model):
    def __init__(self, number_of_label_types=11, number_of_input_channels: int = 1):
        super().__init__()
        self.reshape0 = Reshape([64, 1])
        self.blocks = []
        ...
        for output_channels in [16, 32, 64, 128, 256, 512]:
            ...
            input_channels = output_channels
        self.final_pooling = AveragePooling1D(pool_size=1)
        self.prediction_layer = Convolution1D(number_of_label_types, kernel_size=1, activation=None)
        self.scaling_layer = Convolution1D(number_of_label_types, kernel_size=1, activation=None)
        self.reshape = Reshape([number_of_label_types])
...
class MiraNoBnNoDoNoSigmoidOnPredictLayerLowFinalLayerInitialization(Model):
    def __init__(self, number_of_label_types=11, number_of_input_channels: int = 1):
        super().__init__()
        self.reshape0 = Reshape([64, 1])
        self.blocks = []
        ...
        self.final_pooling = AveragePooling1D(pool_size=1)
        self.prediction_layer = Convolution1D(number_of_label_types, kernel_size=1, activation=None,
                                              kernel_initializer=TruncatedNormal(stddev=0.0000001))
        self.scaling_layer = Convolution1D(number_of_label_types, kernel_size=1, activation=None,
                                           kernel_initializer=TruncatedNormal(stddev=0.0000001))
        self.reshape = Reshape([number_of_label_types])
...
class MiraNoSigmoidLowFinalLayerInitialization(Model):
    def __init__(self, number_of_label_types=11, number_of_input_channels: int = 1):
        super().__init__()
        self.reshape0 = Reshape([64, 1])
        self.blocks = []
