from tensorflow import sigmoid, reduce_mean, reduce_max, reduce_min
...
class LiraNoBn(Model):
    def __init__(self, number_of_input_channels: int = 11):
        ...
        l2_rate = 0.0001
        self.blocks.append(ResidualGenerationLightCurveNetworkBlock(
            output_channels=output_channels, input_channels=50, dropout_rate=0.0, l2_regularization=l2_rate, batch_normalization=False))
        input_channels = output_channels
        for output_channels in [256 * 8, 128 * 8, 64 * 8, 32 * 8, 16 * 8, 8 * 8]:
            self.blocks.append(ResidualGenerationLightCurveNetworkBlock(
                output_channels=output_channels, input_channels=input_channels, pooling_size=2, dropout_rate=0.0, l2_regularization=l2_rate, batch_normalization=False))
            for _ in range(2):
                self.blocks.append(ResidualGenerationLightCurveNetworkBlock(output_channels=output_channels,
                                                                            dropout_rate=0.0, l2_regularization=l2_rate, batch_normalization=False))
            input_channels = output_channels
        self.end_conv = Convolution1D(1, kernel_size=1)
        self.reshape = Reshape([64])
...
class LiraNoBnWithDo(Model):
    def __init__(self, number_of_input_channels: int = 11):
        ...
        l2_rate = 0.0001
        self.blocks.append(ResidualGenerationLightCurveNetworkBlock(
            output_channels=output_channels, input_channels=50, dropout_rate=0.1, l2_regularization=l2_rate, batch_normalization=False))
        input_channels = output_channels
        for output_channels in [256 * 8, 128 * 8, 64 * 8, 32 * 8, 16 * 8, 8 * 8]:
            self.blocks.append(ResidualGenerationLightCurveNetworkBlock(
                output_channels=output_channels, input_channels=input_channels, pooling_size=2, dropout_rate=0.1, l2_regularization=l2_rate, batch_normalization=False))
            for _ in range(2):
                self.blocks.append(ResidualGenerationLightCurveNetworkBlock(output_channels=output_channels,
                                                                            dropout_rate=0.1, l2_regularization=l2_rate, batch_normalization=False))
            input_channels = output_channels
        self.end_conv = Convolution1D(1, kernel_size=1)
        self.reshape = Reshape([64])
