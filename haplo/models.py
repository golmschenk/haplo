...
from torch.nn import Module, Conv1d, LeakyReLU
...
class LiraTraditionalShape8xWidthWith0d5DoNoBn(Module):
    def __init__(self, number_of_input_channels: int = 11):
        super().__init__()
        self.blocks = []
        self.dense0 = Conv1d(1, 400, kernel_size=1)
...
class ResidualGenerationLightCurveNetworkBlock(Module):
    def __init__(self, output_channels: int, input_channels: Optional[int] = None, kernel_size: int = 3,
                 ...
                 l2_regularization: float = 0.0, leaky_relu_negative_slope: float = 0.01, renorm: bool = False):
        super().__init__()
        leaky_relu = LeakyReLU(negative_slope=leaky_relu_negative_slope)
