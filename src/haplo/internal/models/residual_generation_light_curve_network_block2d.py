...
from torch.nn import Module, GELU, BatchNorm2d, Upsample, ConvTranspose2d, \
    ConstantPad2d, Dropout2d
    ...
    def __init__(self, input_channels: int, output_channels: int, kernel_size: int = 3,
        ...
        if output_channels != input_channels:
            if output_channels < input_channels:
                ...
                self.dimension_change_layer = ConstantPad2d(padding=(0, 0, 0, -self.input_to_output_channel_difference),
                                                            value=0)
        else:
