...
from torch.nn import Module, ModuleList, Conv2d, LeakyReLU, Identity
...
class Cura(Module):
    ...
    def __init__(self, number_of_input_features: int, input_transformation: Module, output_transformation: Module):
        ...
        self.dense0 = Conv2d(self.number_of_input_features, 400, kernel_size=1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x = x.reshape([-1, self.number_of_input_features, 1, 1])
        x = self.dense0(x)
        x = self.activation(x)
        x = self.dense1(x)
        x = self.activation(x)
